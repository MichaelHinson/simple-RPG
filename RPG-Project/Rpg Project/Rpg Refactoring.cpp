...
{	
	if (hero.GetName() == "NULL NAME")//If default name sends user to namer, helps with loading.
		characterNamer(hero);
...
int main()
{
	...
	if (saveData.fail())//If no save data, start game with basic stats
	{
		...
		Character Hero1("NULL NAME", 20, 3, 5, 80, 5, 3, .2, 5, 0, 0, 1);
		saveData.close();
		initializer(Hero1);
	}
...
int deathScreen(Character& hero)//If Hero reaches 0 health during fight() this screen is displayed showing monsters killed
{
	...
	{
		Character Hero("NULL NAME", 20, 3, 5, 80, 5, 3, .2, 5, hero.GetGold(), hero.GetExp()/2, 1);//Hero recieves gold of previous hero and 1/2 of xp
...
void heroStatSelection(Character& hero)//Allows the user to improve the hero upon leveling up
{
	...
	case '1':
		...
		cout << hero.GetName() << " now has " << hero.GetCurrentHealth() << "/" << hero.GetMaxHealth() << " Health" << endl;
		break;
	case '2':
