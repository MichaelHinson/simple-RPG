...
Character Skeleton("Skeleton", 6, 2, 65, 0);
Character Zombie("Zombie", 10, 5, 30, .25);
Character Slime("Slime", 8, 1, 70, 0);
Character Imp("Imp", 2, 3, 50, 0);
int main()
{
	srand(time(NULL));
	//monsterCreation();
...
void fightPicker()
{
	...
	if (fightDeterminant == 1)
	{
		fight(Hero1.GetHealth(), Hero1.GetAttack(), Hero1.GetHitChance(), Hero1.GetDefense(),
			Skeleton.GetName(),Skeleton.GetHealth(), Skeleton.GetAttack(), Skeleton.GetHitChance(),
			Skeleton.GetDefense());
	}
	else if (fightDeterminant == 2)
	{
		fight(Hero1.GetHealth(), Hero1.GetAttack(), Hero1.GetHitChance(), Hero1.GetDefense(),
			Slime.GetName(), Slime.GetHealth(), Slime.GetAttack(), Slime.GetHitChance(),
			Slime.GetDefense());
	}
	else if (fightDeterminant == 3)
	{
		fight(Hero1.GetHealth(), Hero1.GetAttack(), Hero1.GetHitChance(), Hero1.GetDefense(),
			Zombie.GetName(), Zombie.GetHealth(), Zombie.GetAttack(), Zombie.GetHitChance(),
			Zombie.GetDefense());
	}
	else if (fightDeterminant == 4)
	{
		fight(Hero1.GetHealth(), Hero1.GetAttack(), Hero1.GetHitChance(), Hero1.GetDefense(),
			Imp.GetName(), Imp.GetHealth(), Imp.GetAttack(), Imp.GetHitChance(),
			Imp.GetDefense());
	}
}
void itemShop()
